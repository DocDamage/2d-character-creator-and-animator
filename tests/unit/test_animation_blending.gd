...
func run_tests() -> int:
	...
	var base := {"arm_r": {"position": [0.0, 0.0], "rotation": 0.0}, "chest": {"position": [0.0, 0.0], "scale": [1.0, 1.0]}, "weapon": {"rotation": 0.0}}
	var poses := {"aim": {"arm_r": {"position": [10.0, 0.0], "rotation": 1.0}, "leg": {"position": [9.0, 0.0]}}, "breathing": {"chest": {"position": [0.0, 2.0], "scale": [1.2, 0.8]}}, "sword": {"weapon": {"rotation": 0.75}}}
	...
	if upper_ok and additive_ok and weapon_ok and absf(float(result.pose.arm_r.position[0]) - 5.0) < 0.001 and absf(float(result.pose.chest.position[1]) - 1.0) < 0.001 and absf(float(result.pose.chest.scale[0]) - 1.1) < 0.001 and absf(float(result.pose.chest.scale[1]) - 0.9) < 0.001 and not result.pose.has("leg") and absf(float(result.pose.weapon.rotation)) < 0.001 and absf(float(included.pose.weapon.rotation) - 0.75) < 0.001 and absf(float(result.sync_times.upper) - 0.4) < 0.001 and restored.validate().is_empty():
		print("  PASS: BLD-001 through BLD-005 masks, additive layers (including scale), sync groups, and weapon overlays blend deterministically")
		return 1
